<?php
	...
	//-----------------------------------------------------------------------------------
	if($_POST['action']=='Add'){
				$query="INSERT INTO `tbl_menu_user` (`id` ,`userID` ,`projectID`)VALUES ('' , '".$_GET['id']."', '".$_POST['projectID']."') ";
				mysql_query($query);
		}
		if($_POST['action']=='delete'){
			$query="DELETE FROM  tbl_menu_user WHERE id =  '".$_POST['editID']."' ";
				mysql_query($query);
		}
	//------------------------project memeber list-------------------//userID 	projectID 
	$query="SELECT a.*,  m.admin_name , p.menuName FROM tbl_menu_user a "
...
<span class="header1">รายละเอียดการใช้งาน</span>
<hr size="1" class="hr"  width="98%"/>
เพิ่มเมนูการใช้งาน 
<select name="projectID" id="projectID">
          <?php while($data=mysql_fetch_assoc($resultCate)){ ?>
            <option value="<?php echo $data['id']?>"><?php echo $data['menuName']?></option>
            <?php } ?>
...
<table width="520" border="0" cellpadding="0" cellspacing="0">
  <tr class="txtDetail12">
    <td width="459" height="25" align="center" bgcolor="#DADADA">เมนุการใช้งาน</td>
    <td width="61" align="center" bgcolor="#DADADA">ลบ</td>
  </tr>
  <?php while($data=mysql_fetch_assoc($resultProject)){ ?>
  <tr>
    <td height="30" class="txtDetail12">&nbsp;<?php echo $data['menuName']?></td>
